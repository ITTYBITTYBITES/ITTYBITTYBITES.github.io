    ...
    const wrapper = h('div', { class: 'yearglass-preview publication-panel', style: 'padding: var(--space-6); max-width: 100%; min-height: 100vh; display: flex; flex-direction: column; justify-content: center;; overflow-x: hidden; font-size: 1rem; line-height: 1.5;' }, [
      ...
      h('section', { class: 'section preview-section' }, [
        ...
        h('div', { style: 'margin-top: var(--space-4);', onclick: 'initAudio(); unlockAudio();' }, [
          h('div', {
            `initAudio(); unlockAudio(); playTone(880, 0.3, 0.3); const el=document.createElement('div'); el.style.cssText='position:fixed;inset:0;z-index:9999;background:linear-gradient(135deg,#0d0d0e 0%,#1a1814 60%,#0a1a1a 100%);color:#f0ede8;display:flex;flex-direction:column;align-items:center;justify-content:center;font-family:system-ui,sans-serif;animation:fadeIn 0.8s ease'; el.innerHTML='<style>@keyframes fadeIn{from{opacity:0;transform:scale(0.92)}to{opacity:1;transform:scale(1)}}@keyframes reveal{from{opacity:0;transform:scale(0.95);filter:brightness(0.6)}to{opacity:1;transform:scale(1);filter:brightness(1)}}</style><div style=text-align:center;padding:var(--space-6);max-width:720px;animation:reveal 1.2s ease-out;><h2 style=font-size:2.5rem;margin-bottom:var(--space-4);color:#bfa06a;>The Dome Opens</h2><p style=font-size:1rem;opacity:0.8;margin-bottom:var(--space-4);>Observe carefully.</p><div id=obsTimer style=font-size:2rem;color:#bfa06a;margin-bottom:var(--space-6);>00:00</div><button onclick="this.closest(\'[style*=z-index]\').remove()" style=padding:0.75rem 1.75rem;border-radius:8px;border:1px solid #bfa06a;background:#bfa06a;color:#0d0d0e;font-weight:700;font-size:1rem;cursor:pointer;box-shadow:0 8px 24px rgba(191,160,106,0.25);>Return to Room</button></div>'; document.body.appendChild(el); let s=0; const timer=setInterval(()=>{s++; const m=Math.floor(s/60).toString().padStart(2,'0'); const sec=(s%60).toString().padStart(2,'0'); const t=document.getElementById('obsTimer'); if(t)t.textContent=m+':' +sec;},1000);`,

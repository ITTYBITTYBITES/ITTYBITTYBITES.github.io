...
const BE_PATH = path.join(__dirname, 'build-engine.js');
const UP_PATH = path.join(__dirname, 'upgrade_html_architecture.js');

const masterFooter = `    <!-- UNIFIED LEGAL & MISSION FOOTER -->
        ...
        <div class="max-w-[1400px] mx-auto grid grid-cols-1 md:grid-cols-4 gap-10 mb-16 border-b border-slate-900 pb-16 text-left">
            ...
            <div>
                ...
                <div class="flex flex-col space-y-3 mt-2">
                    <a href="https://play.google.com/store/apps/details?id=com.ittybittybites.the2secondwitness" target="_blank" rel="noopener" class="px-5 py-3.5 bg-cyan-500/10 hover:bg-cyan-500 hover:text-slate-950 text-cyan-400 border border-cyan-400/30 rounded-xl font-bold transition-all flex items-center justify-between text-xs shadow-lg tracking-wider font-mono">
                        <span>🚀 ANDROID APP</span> <span>&rarr;</span>
                    </a>
                    <a href="https://ittybittybites.itch.io/2-second-witness" target="_blank" rel="noopener" class="px-5 py-3.5 bg-gold/10 hover:bg-gold hover:text-slate-950 text-gold border border-gold/30 rounded-xl font-bold transition-all flex items-center justify-between text-xs shadow-lg tracking-wider font-mono">
...
// ─── build-engine.js ──────────────────────────────────────────────────────────
let be = fs.readFileSync(BE_PATH, 'utf8');

// Custom dynamic Footer builder in build-engine.js
const makeFooterFunc = `function makeGlobalFooter(prefix = '') {\n  return \`${masterFooter}\`.replace(/\\{\\{PREFIX\\}\\}/g, prefix);\n}`;
if (!be.includes('function makeGlobalFooter(')) {
    be += `\n${makeFooterFunc}\n`;
}

// 1. Fully replace Footers in buildIntelPageTemplate (lines 834-897)
const oldFooterBlock1 = /<footer class="border-t border-slate-900 bg-slate-950 py-16 px-6 mt-auto text-slate-500 font-mono text-xs leading-relaxed select-none">[\s\S]*?<\/footer>/;
if (be.match(oldFooterBlock1)) {
    be = be.replace(oldFooterBlock1, "${makeGlobalFooter('../')}");
    console.log("✓ Upgraded Footer in buildIntelPageTemplate");
}

// 2. Upgraded generateIntelHub (lines 904-1045)
const fullOldGenHub = /function generateIntelHub\(topics, personas, intelPages\) \{[\s\S]*?console\.log\(`  ✓ Cleaned .*? existing intel hub pages`\);[\s\S]*?<\/html>`;\n\n  fs\.writeFileSync\(path\.join\(INTEL_DIR, 'index\.html'\), hubHtml, 'utf-8'\);\n\}/m;

const upgradedGenHub = `function generateIntelHub(topics, personas, intelPages) {
  console.log('\\n📊 Generating definitive intel hub index (with Table, Accordion, Search & Filter capabilities)...');
  ...
  const topicBlocks = topics.map(topic => {
    ...
    const pageRows = pagesForTopic.map(p => {
      ...
      return \`
        <a href="\${p.slug}.html" class="matrix-item-card bg-slate-900/40 border border-slate-800/80 rounded-xl p-4 hover:border-cyan-400 hover:bg-slate-900 transition-all flex items-center justify-between text-xs font-mono group shadow-md" data-title="\${(persona?.name || p.persona).toLowerCase()}">
          <span class="text-slate-300 font-bold group-hover:text-white flex items-center gap-2">
            <span class="text-cyan-400">◈</span> <span class="truncate">\${persona?.name || p.persona}</span>
          </span>
          <span class="text-cyan-400 font-bold tracking-widest uppercase group-hover:translate-x-1 transition-transform shrink-0">Protocol &rarr;</span>
        </a>\`;
    }).join('');

    return \`
    <div class="topic-matrix-section space-y-4 text-left" data-cat="\${(topic.category || topic.universe || 'TECH').toUpperCase()}" data-topic="\${topic.title.toLowerCase()} \${topic.focus?.toLowerCase() || ''}">
...
<title>Definitive Intel Matrix | Authoritative Operator Defenses</title>
<meta name="description" content="Programmatic cognitive training protocol matrix — \${intelPages.length} briefings across \${topics.length} topics and \${personas.length} personas. Live Search and Dynamic Categories.">
...
<script src="../portal.js?v=202606182330"></script>
<style>
...
<section class="py-16 text-center bg-zinc-950 border-b border-slate-900 select-none shrink-0">
  <div class="max-w-[1100px] mx-auto px-5 font-mono space-y-4">
    ...
    <p class="text-slate-300 text-xs sm:text-sm max-w-2xl mx-auto leading-relaxed font-mono">
        An authoritative, fully searchable database of tactical cognitive override protocols. Use the interactive live search or category tracks below to instantly unmask designated neural defenses.
    </p>
    ...
    <div class="max-w-xl mx-auto relative pt-2">
      ...
      <input id="matrix-search-box" type="text" placeholder="◈ Filter Exactly \${intelPages.length} operational briefings by keyword or operator title..." class="w-full bg-slate-900/90 border-2 border-slate-800 focus:border-cyan-400 rounded-2xl py-4 pl-12 pr-6 text-xs text-white placeholder:text-slate-500 tracking-wider font-mono outline-none transition-all shadow-2xl">
    </div>
  </div>
</section>

<!-- Filter Track -->
<section class="bg-slate-950 py-6 border-b border-slate-900 select-none shrink-0">
...
\${makeGlobalFooter('../')}
  ...
  fs.writeFileSync(path.join(INTEL_DIR, 'index.html'), hubHtml, 'utf-8');
  console.log('  ✓ Definitive Intel matrix hub generated beautifully with Accordion, Search and Category filters.');
}`;

if (be.includes('function generateIntelHub(topics, personas, intelPages) {')) {
    be = be.replace(fullOldGenHub, upgradedGenHub);
    console.log("✓ Upgraded generateIntelHub in build-engine.js");
}

// 3. Upgraded updateDynamicShowcases to enforce True Daily Rotator (lines 1092-1135)
const fullOldShowcase = /function updateDynamicShowcases\(intelPages\) \{[\s\S]*?<\/section>`;/m;
const upgradedShowcase = `function updateDynamicShowcases(intelPages) {
...
if (be.includes('function updateDynamicShowcases(intelPages) {')) {
    be = be.replace(fullOldShowcase, upgradedShowcase);
    console.log("✓ Upgraded updateDynamicShowcases in build-engine.js");
}

fs.writeFileSync(BE_PATH, be, 'utf8');

// ─── upgrade_html_architecture.js ─────────────────────────────────────────────
let up = fs.readFileSync(UP_PATH, 'utf8');

const upMakeFooter = `function makeGlobalFooter(prefix = '') {\n  return \`${masterFooter}\`.replace(/\\{\\{PREFIX\\}\\}/g, prefix);\n}`;

if (up.includes('const GLOBAL_FOOTER = `')) {
    const fullOldUpFooter = /const GLOBAL_FOOTER = `[\s\S]*?<\/footer>`;/m;
    up = up.replace(fullOldUpFooter, upMakeFooter);
    console.log("✓ Upgraded Footer in upgrade_html_architecture.js");
}

const oldFullCall = `    let fullHtml = part1 + freshNav + content.slice(endNav) + '\\n' + GLOBAL_FOOTER;`;
const newFullCall = `    let prefix = (filename === 'sitemap.xml' || filename.includes('/')) ? '../' : '';\n    let fullHtml = part1 + freshNav + content.slice(endNav) + '\\n' + makeGlobalFooter(prefix);`;
if (up.includes(oldFullCall)) {
    up = up.replace(oldFullCall, newFullCall);
} else if (up.includes('GLOBAL_FOOTER')) {
    up = up.replace(/GLOBAL_FOOTER/g, "makeGlobalFooter('')");
}

fs.writeFileSync(UP_PATH, up, 'utf8');
